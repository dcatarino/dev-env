...
## Prerequisite: branch matches the identifier

Check the current branch (`git branch --show-current`) before committing. If
the branch name contains a different identifier than the commit's (e.g.
committing `task-13310` while on `18.0-task-14435`), **stop and ask** which
branch to use — do not commit onto a mismatched branch on your own.
...
- The **first run in a fresh codespace is slow** (it installs the hook
  environments, which can take minutes). Run it as a background task and wait
  for its completion notification — never foreground-`sleep` or poll.
